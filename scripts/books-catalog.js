(function () {
  ...
  const PLACEHOLDER_COVER = "../assets/aeswp.jpg";
  ...
  function deriveSalesCoverFromUrl(salesUrl) {
    const idMatch = salesUrl.match(/id(\d+)/i);
    if (!idMatch) {
      return null;
    }

    return `https://itunes.apple.com/lookup?id=${idMatch[1]}&entity=ebook`;
  }

  async function resolveCover(book) {
    const localCover = COVER_ASSET_MAP[book.slug];
    if (localCover) {
      return localCover;
    }

    const lookupUrl = deriveSalesCoverFromUrl(book.salesUrl);
    if (!lookupUrl) {
    ...
    try {
      ...
      return first.artworkUrl512 || first.artworkUrl100 || PLACEHOLDER_COVER;
    } catch (error) {
      return PLACEHOLDER_COVER;
    }
  }
